        ...
        <div className="flex gap-3 justify-center">
            {Array.from({ length }).map((_, index) => (
                <input key={index} ref={el => inputs[index] = el}
                    ...
                    onKeyDown={e => handleKeyDown(index, e)}
                    onPaste={handlePaste}
                    className="w-12 h-14 text-center text-2xl font-bold bg-dark-800 border-2 border-dark-700 rounded-xl text-dark-100 focus:outline-none focus:border-ai-500 focus:ring-2 focus:ring-ai-500/30 transition-all"
                />
            ))}
        </div>
    );
...
const PasswordRequirement = ({ met, text }) => (
    <div className="flex items-center gap-2 text-xs">
        {met ? <CheckCircle2 size={14} className="text-accent-400 flex-shrink-0" /> : <XCircle size={14} className="text-dark-500 flex-shrink-0" />}
        <span className={met ? 'text-accent-400' : 'text-dark-500'}>{text}</span>
    </div>
);
...
const StepIndicator = ({ currentStep }) => (
    <div className="flex items-center justify-center gap-2 mb-6">
        {[1, 2].map((step) => (
            <div key={step} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${currentStep === step ? 'bg-ai-600 text-white' : currentStep > step ? 'bg-accent-500 text-white' : 'bg-dark-800 text-dark-500'}`}>
                    {currentStep > step ? <CheckCircle2 size={16} /> : step}
                </div>
                {step < 2 && <div className={`w-16 h-0.5 transition-all ${currentStep > step ? 'bg-accent-500' : 'bg-dark-700'}`} />}
            </div>
        ))}
    </div>
);
...
const Register = () => {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'confirmPassword' || (name === 'password' && formData.confirmPassword)) {
            const pwd = name === 'password' ? value : formData.password;
            const confirmPwd = name === 'confirmPassword' ? value : formData.confirmPassword;
            setErrors(prev => ({ ...prev, confirmPassword: confirmPwd && pwd !== confirmPwd ? 'Passwords do not match' : '' }));
        }
    };
    ...
    const handleRegisterAndSendOtp = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post(`${API_URL}/api/auth/otp/send`, { email: formData.email, purpose: 'REGISTER' });
            toast.success(`Verification OTP sent to ${formData.email}`);
            setStep(2);
            startTimer();
        } catch (err) {
    ...
    const handleVerifyAndCreate = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.post(`${API_URL}/api/auth/otp/verify-register`, { ...formData, otp });
    ...
    const handleGoogleSignup = () => { window.location.href = `${API_URL}/oauth2/authorization/google`; };
        ...
        <div className="min-h-screen bg-dark-950 flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-2xl relative z-10">
                ...
                <Card className="shadow-2xl">
                    ...
                    <AnimatePresence mode="wait">
                        {step === 1 && (
                            <motion.form key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} onSubmit={handleRegisterAndSendOtp} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Input type="text" name="name" placeholder="Full name" label="Full Name" icon={User} value={formData.name} onChange={handleChange} required error={errors.name} />
                                    <Input type="email" name="email" placeholder="your@email.com" label="Email" icon={Mail} value={formData.email} onChange={handleChange} required error={errors.email} />
                                </div>
                                <div className="relative">
                                    <Input type={showPassword ? 'text' : 'password'} name="password" placeholder="Create a strong password" label="Password" icon={Lock} value={formData.password} onChange={handleChange} required error={errors.password} />
                                    <button type="button" onClick={() => setShowPassword(p => !p)} className="absolute right-3 top-9 text-dark-400 hover:text-dark-200">
                                        {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                                    </button>
                                </div>
                                {formData.password && (
                                ...
                                <div className="relative">
                                    <Input type={showConfirmPassword ? 'text' : 'password'} name="confirmPassword" placeholder="Re-enter your password" label="Confirm Password" icon={Lock} value={formData.confirmPassword} onChange={handleChange} required error={errors.confirmPassword} />
                                    <button type="button" onClick={() => setShowConfirmPassword(p => !p)} className="absolute right-3 top-9 text-dark-400 hover:text-dark-200">
                                        {showConfirmPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                                    </button>
                                </div>
                                {formData.confirmPassword && formData.password === formData.confirmPassword && !errors.confirmPassword && (
                                    <div className="flex items-center gap-2 text-sm text-accent-400"><CheckCircle2 size={16} /><span>Passwords match</span></div>
                                )}
                                <div className="flex items-center gap-3 py-1">
                                    <div className="flex-1 h-px bg-dark-700" /><span className="text-xs text-dark-500">Location Details</span><div className="flex-1 h-px bg-dark-700" />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <Input type="text" name="city" placeholder="Your city" label="City" icon={MapPin} value={formData.city} onChange={handleChange} required error={errors.city} />
                                    <Input type="text" name="state" placeholder="Your state" label="State" icon={MapPin} value={formData.state} onChange={handleChange} required error={errors.state} />
                                </div>
                                <Input type="text" name="country" placeholder="Country" label="Country" icon={MapPin} value={formData.country} onChange={handleChange} required />
                                <div className="flex items-start gap-2 p-3 bg-ai-500/10 border border-ai-500/20 rounded-lg">
                                    <Mail className="w-4 h-4 text-ai-400 mt-0.5 flex-shrink-0" />
                                    <p className="text-xs text-ai-300">A 6-digit verification code will be sent to your email after clicking Register</p>
                                </div>
                                <Button type="submit" variant="primary" className="w-full" loading={loading}>
                                    {loading ? 'Sending verification code...' : 'Register & Verify Email →'}
                                </Button>
                                <div className="flex items-center gap-4">
                                    <div className="flex-1 h-px bg-dark-800" /><span className="text-sm text-dark-500">or</span><div className="flex-1 h-px bg-dark-800" />
                                </div>
                                <Button variant="secondary" className="w-full" onClick={handleGoogleSignup}>
                                    <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                                        <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                                ...
                                <p className="text-center text-sm text-dark-400">
                                    Already have an account?{' '}
                                    <Link to="/login" className="text-ai-400 hover:text-ai-300 font-medium transition-colors">Sign in</Link>
                                </p>
                            </motion.form>
                        )}
                        {step === 2 && (
                            <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }} className="space-y-6">
                                ...
                                <OtpInput value={otp} onChange={setOtp} length={6} />
                                <Button variant="primary" className="w-full" onClick={handleVerifyAndCreate} disabled={loading || otp.length !== 6} loading={loading}>
                                    <CheckCircle className="w-4 h-4 mr-2" />
                                    {loading ? 'Creating Account...' : 'Verify & Create Account'}
                                </Button>
                                <div className="flex items-center justify-between text-sm">
                                    <button onClick={() => { setStep(1); setOtp(''); }} className="text-dark-400 hover:text-dark-200 transition-colors">← Edit details</button>
                                    <button onClick={handleResend} disabled={resendTimer > 0 || loading} className="inline-flex items-center gap-1.5 text-ai-400 hover:text-ai-300 disabled:opacity-50 transition-colors">
                                        <RefreshCw className="w-3.5 h-3.5" />{resendTimer > 0 ? `Resend in ${resendTimer}s` : 'Resend OTP'}
                                    </button>
                                </div>
                                <div className="p-3 bg-dark-800/50 rounded-lg border border-dark-700 text-xs text-dark-400 space-y-1">
                                    <p>📧 <span className="text-dark-300">{formData.email}</span></p>
                ...
                <p className="text-center text-xs text-dark-500 mt-6">By creating an account, you agree to our Terms of Service and Privacy Policy</p>
            </motion.div>
        </div>
    );
};
